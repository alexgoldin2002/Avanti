  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', flexDirection: 'column', ...s }}>
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '48px 24px', flex: 1 }}>
        ...
        <p style={{ fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--muted-foreground)', margin: '0 0 12px' }}>How it works</p>
        <h1 style={{ fontSize: '48px', fontWeight: 300, color: 'var(--forest-deep)', margin: '0 0 16px', lineHeight: 1.1, ...s }}>The best trips start here.</h1>
        <p style={{ fontSize: '16px', color: 'var(--muted-foreground)', margin: '0 0 64px', lineHeight: 1.7 }}>Avanti is the AI travel companion that thinks like a great travel agent — and coordinates your whole group.</p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          {steps.map((step, i) => (
            <div key={step.number} style={{ display: 'flex', gap: '32px', paddingBottom: '40px', borderLeft: i < steps.length - 1 ? '0.5px solid var(--border)' : 'none', marginLeft: '20px', paddingLeft: '32px', position: 'relative' }}>
              <div style={{ position: 'absolute', left: '-20px', top: '0', width: '40px', height: '40px', borderRadius: '50%', background: 'var(--forest-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: '13px', color: '#fff', fontWeight: 300, ...s }}>{step.number}</span>
              </div>
              <div style={{ paddingTop: '8px' }}>
                <h3 style={{ fontSize: '22px', fontWeight: 300, color: 'var(--forest-deep)', margin: '0 0 10px', ...s }}>{step.title}</h3>
                <p style={{ fontSize: '14px', color: 'var(--muted-foreground)', margin: 0, lineHeight: 1.8 }}>{step.description}</p>
              </div>
            </div>
          ))}
        </div>

        <div style={{ background: 'var(--forest-deep)', borderRadius: '12px', padding: '40px', textAlign: 'center', marginTop: '40px' }}>
          ...
          <Link href="/" style={{ display: 'inline-block', background: '#fff', color: 'var(--forest-deep)', padding: '14px 32px', fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', textDecoration: 'none', borderRadius: '8px', fontFamily: 'var(--font-cormorant), Georgia, serif' }}>
            Get started →
          </Link>
        </div>
      </div>
